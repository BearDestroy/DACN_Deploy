...
import { layKhoaHocSearch, type KhoaHocSearchParams } from '@/apis/khoahoc'
...
export function TrangTimKiem() {
  ...
  const thamSoApi: KhoaHocSearchParams = {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ['khoaHocTimKiem', tuKhoa, danhMuc, trangHienTai, kichThuocTrang, sapXep, trinhDoLoc],
    queryFn: () => layKhoaHocSearch(thamSoApi),
        ...
        <div className='max-w-[1340px] mx-auto px-6 py-10'>
          <div className='flex flex-col gap-3'>
            ...
            <p className='text-gray-600 text-lg'>
              Tìm thấy <span className='font-bold text-black'>{tongSoKetQua}</span> khóa học phù hợp.
            </p>
          </div>
        </div>
      </div>
          ...
          {isLoading ? (
            ...
            <>
              <div className='grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 animate-in fade-in duration-500 slide-in-from-bottom-4'>
                {danhSachKhoaHoc.map((khoaHoc: KhoaHocChiTietHomePage) => (
            ...
            <div className='flex flex-col items-center justify-center py-24 bg-white rounded-sm border border-gray-200 shadow-sm'>
              ...
              <h3 className='text-2xl font-bold text-[#2d2f31] mb-3'>Không tìm thấy khóa học nào</h3>
