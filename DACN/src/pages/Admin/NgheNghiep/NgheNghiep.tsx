...
import { layDSNgheNghiep, xoaNgheNghiep } from '@/apis/nghenghiep'
import { useMutation, useQuery } from '@tanstack/react-query'
import { useQueryString } from '@/hooks/use-query-string'
import { useLocation, useNavigate } from 'react-router-dom'
import NgheNghiepForm from '@/components/Form/NgheNghiepForm'
...
export function QuanLyNgheNghiep() {
  ...
  const { data, refetch, isLoading } = useQuery({
    queryKey: ['dsNgheNghiep', soTrang, soLuong, trangThai, tuKhoa],
    queryFn: () => layDSNgheNghiep(soTrang, soLuong, trangThai, tuKhoa)
  })
  ...
  const [editingNgheNghiep, setEditingNgheNghiep] = useState<ngheNghiepFormData | undefined>(undefined)
  const [selectedRowId, setSelectedRowId] = useState<number | null>(null)
  const handleSetEditingNgheNghiep = (data: NgheNghiep) => {
    setEditingNgheNghiep({
      tenNgheNghiep: data.tenNgheNghiep,
      maNgheNghiep: data.maNgheNghiep,
  ...
  const handleFormSuccess = () => {
    ...
    setEditingNgheNghiep(undefined)
    refetch()
  }
  ...
  const { mutate: deleteMutate, isPending: isDeleting } = useMutation({
    mutationFn: ({ id }: { id: number }) => xoaNgheNghiep(id),
    onSuccess: (data) => {
    ...
    <div className='relative min-h-screen w-full bg-gray-50 p-4 flex flex-col'>
      ...
      <div className='flex-1 grid grid-cols-1 lg:grid-cols-3 gap-6 w-full'>
        <div className='lg:col-span-2 bg-white rounded-lg border border-gray-200 p-6 w-full shadow-sm flex flex-col'>
          <div className='space-y-3'>
            <div className='flex gap-2'>
              <Input
                placeholder='Tìm kiếm theo mã hoặc tên nghề nghiệp...'
                value={searchQuery}
                ...
                {ketQua.length === 0 ? (
                  ...
                  ketQua.map((s, i) => (
                    ...
                    >
                      ...
                      <TooltipTableCell text={s.tenNgheNghiep} index={i} className='text-base py-3' />
                      <TableCell className='text-center border text-base py-0.5 border-gray-200 border-b '>
        ...
        <div className='bg-white rounded-lg border p-6 w-full shadow-sm'>
          <NgheNghiepForm
            defaultValues={editingNgheNghiep}
