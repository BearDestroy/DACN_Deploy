...
import { layDSMucDich, xoaMucDich } from '@/apis/mucdich'
import { useMutation, useQuery } from '@tanstack/react-query'
import { useQueryString } from '@/hooks/use-query-string'
import { useLocation, useNavigate } from 'react-router-dom'
import MucDichForm from '@/components/Form/MucDichForm'
...
export function QuanLyMucDich() {
  ...
  const { data, refetch, isLoading } = useQuery({
    queryKey: ['dsMucDich', soTrang, soLuong, trangThai, tuKhoa],
    queryFn: () => layDSMucDich(soTrang, soLuong, trangThai, tuKhoa)
  })
  ...
  const [editingMucDich, setEditingMucDich] = useState<mucDichFormData | undefined>(undefined)
  const [selectedRowId, setSelectedRowId] = useState<number | null>(null)
  const handleSetEditingMucDich = (data: MucDich) => {
    setEditingMucDich({
      tenMucDich: data.tenMucDich,
      maMucDich: data.maMucDich,
  ...
  const { mutate: deleteMutate, isPending: isDeleting } = useMutation({
    mutationFn: ({ id }: { id: number }) => xoaMucDich(id),
    onSuccess: (data) => {
        ...
        <div className='lg:col-span-2 bg-white rounded-lg border border-gray-200 p-6 w-full shadow-sm flex flex-col'>
          <div className='space-y-3'>
            <div className='flex gap-2'>
              <Input
                placeholder='Tìm kiếm theo mã hoặc tên mục đích...'
                value={searchQuery}
                ...
                {ketQua.length === 0 ? (
                  ...
                  ketQua.map((s, i) => (
                    ...
                    >
                      ...
                      <TooltipTableCell text={s.tenMucDich} index={i} className='text-base py-3' />
                      <TableCell className='text-center border text-base py-0.5 border-gray-200 border-b '>
