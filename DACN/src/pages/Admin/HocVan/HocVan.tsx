...
import { layDSHocVan, xoaHocVan } from '@/apis/hocvan'
import { useMutation, useQuery } from '@tanstack/react-query'
import { useQueryString } from '@/hooks/use-query-string'
import { useLocation, useNavigate } from 'react-router-dom'
import HocVanForm from '@/components/Form/HocVanForm'
...
export function QuanLyHocVan() {
  ...
  const { data, refetch, isLoading } = useQuery({
    queryKey: ['dsHocVan', soTrang, soLuong, trangThai, tuKhoa],
    queryFn: () => layDSHocVan(soTrang, soLuong, trangThai, tuKhoa)
  })
  ...
  const [editingHocVan, setEditingHocVan] = useState<hocVanFormData | undefined>(undefined)
  const [selectedRowId, setSelectedRowId] = useState<number | null>(null)
  const handleSetEditingHocVan = (data: HocVan) => {
    setEditingHocVan({
      tenHocVan: data.tenHocVan,
      maHocVan: data.maHocVan,
  ...
  const { mutate: deleteMutate, isPending: isDeleting } = useMutation({
    mutationFn: ({ id }: { id: number }) => xoaHocVan(id),
    onSuccess: (data) => {
        ...
        <div className='lg:col-span-2 bg-white rounded-lg border border-gray-200 p-6 w-full shadow-sm flex flex-col'>
          <div className='space-y-3'>
            <div className='flex gap-2'>
              <Input
                placeholder='Tìm kiếm theo mã hoặc tên học vấn...'
                value={searchQuery}
                ...
                {ketQua.length === 0 ? (
                  ...
                  ketQua.map((s, i) => (
                    ...
                    >
                      ...
                      <TooltipTableCell text={s.tenHocVan} index={i} className='text-base py-3' />
                      <TableCell className='text-center border text-base py-0.5 border-gray-200 border-b '>
