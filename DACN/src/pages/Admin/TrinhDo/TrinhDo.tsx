...
import { layDSTrinhDo, xoaTrinhDo } from '@/apis/trinhdo'
import { useMutation, useQuery } from '@tanstack/react-query'
import { useQueryString } from '@/hooks/use-query-string'
import { useLocation, useNavigate } from 'react-router-dom'
import TrinhDoForm from '@/components/Form/TrinhDoForm'
...
export function QuanLyTrinhDo() {
  ...
  const { data, refetch, isLoading } = useQuery({
    queryKey: ['dsTrinhDo', soTrang, soLuong, trangThai, tuKhoa],
    queryFn: () => layDSTrinhDo(soTrang, soLuong, trangThai, tuKhoa)
  })
  ...
  const [editingTrinhDo, setEditingTrinhDo] = useState<trinhDoFormData | undefined>(undefined)
  const [selectedRowId, setSelectedRowId] = useState<number | null>(null)
  const handleSetEditingTrinhDo = (data: TrinhDo) => {
    setEditingTrinhDo({
      tenTrinhDo: data.tenTrinhDo,
  ...
  const { mutate: deleteMutate, isPending: isDeleting } = useMutation({
    mutationFn: ({ id }: { id: number }) => xoaTrinhDo(id),
    onSuccess: (data) => {
        ...
        <div className='lg:col-span-2 bg-white rounded-lg border border-gray-200 p-6 w-full shadow-sm flex flex-col'>
          <div className='space-y-3'>
            <div className='flex gap-2'>
              <Input
                placeholder='Tìm kiếm theo mã hoặc tên trình độ...'
                value={searchQuery}
                ...
                {ketQua.length === 0 ? (
                  ...
                  ketQua.map((s, i) => (
                    ...
                    >
                      ...
                      <TooltipTableCell text={s.tenTrinhDo} index={i} className='text-base py-3' />
                      <TableCell className='text-center border text-base py-0.5 border-gray-200 border-b '>
