...
import { layDSDoKho, xoaDoKho } from '@/apis/dokho.api'
import { useMutation, useQuery } from '@tanstack/react-query'
import { useQueryString } from '@/hooks/use-query-string'
import { useLocation, useNavigate } from 'react-router-dom'
import DoKhoForm from '@/components/Form/DoKhoForm'
...
export function QuanLyDoKho() {
  ...
  const { data, refetch, isLoading } = useQuery({
    queryKey: ['dsDoKho', soTrang, soLuong, trangThai, tuKhoa],
    queryFn: () => layDSDoKho(soTrang, soLuong, trangThai, tuKhoa)
  })
  ...
  const [editingDoKho, setEditingDoKho] = useState<doKhoFormData | undefined>(undefined)
  const [selectedRowId, setSelectedRowId] = useState<number | null>(null)
  const handleSetEditingDoKho = (data: DoKho) => {
    setEditingDoKho({
      tenDoKho: data.tenDoKho,
      maDoKho: data.maDoKho,
  ...
  const { mutate: deleteMutate, isPending: isDeleting } = useMutation({
    mutationFn: ({ id }: { id: number }) => xoaDoKho(id),
    onSuccess: (data) => {
        ...
        <div className='lg:col-span-2 bg-white rounded-lg border border-gray-200 p-6 w-full shadow-sm flex flex-col'>
          <div className='space-y-3'>
            <div className='flex gap-2'>
              <Input
                placeholder='Tìm kiếm theo mã hoặc tên độ khó...'
                value={searchQuery}
                ...
                {ketQua.length === 0 ? (
                  ...
                  ketQua.map((s, i) => (
                    ...
                    >
                      ...
                      <TooltipTableCell text={s.tenDoKho} index={i} className='text-base py-3' />
                      <TableCell className='text-center border text-base py-0.5 border-gray-200 border-b '>
