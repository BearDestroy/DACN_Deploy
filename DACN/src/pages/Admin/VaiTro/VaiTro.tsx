...
import { layDSVaiTro, xoaVaiTro } from '@/apis/vaitro'
import { useMutation, useQuery } from '@tanstack/react-query'
import { useQueryString } from '@/hooks/use-query-string'
import { useLocation, useNavigate } from 'react-router-dom'
import VaiTroForm from '@/components/Form/VaiTroForm'
...
export function QuanLyVaiTro() {
  ...
  const { data, refetch, isLoading } = useQuery({
    queryKey: ['dsVaiTro', soTrang, soLuong, trangThai, tuKhoa],
    queryFn: () => layDSVaiTro(soTrang, soLuong, trangThai, tuKhoa)
  })
  ...
  const [editingVaiTro, setEditingVaiTro] = useState<vaiTroFormData | undefined>(undefined)
  const [selectedRowId, setSelectedRowId] = useState<number | null>(null)
  const handleSetEditingVaiTro = (data: VaiTro) => {
    setEditingVaiTro({
  ...
  const { mutate: deleteMutate, isPending: isDeleting } = useMutation({
    mutationFn: ({ id }: { id: number }) => xoaVaiTro(id),
    onSuccess: (data) => {
                ...
                {ketQua.length === 0 ? (
                  ...
                  ketQua.map((s, i) => (
                    ...
                    >
                      ...
                      <TooltipTableCell text={s.tenVaiTro} index={i} className='text-base py-3' />
                      <TableCell className='text-center border text-base py-0.5 border-gray-200 border-b '>
