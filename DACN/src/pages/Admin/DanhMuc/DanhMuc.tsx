...
import { layDSDanhMuc, xoaDanhMuc } from '@/apis/danhmuc.api'
import { useMutation, useQuery } from '@tanstack/react-query'
import { useQueryString } from '@/hooks/use-query-string'
import { useLocation, useNavigate } from 'react-router-dom'
import DanhMucForm from '@/components/Form/DanhMucForm'
...
export function QuanLyDanhMuc() {
  ...
  const { data, refetch, isLoading } = useQuery({
    queryKey: ['dsDanhMuc', soTrang, soLuong, trangThai, tuKhoa],
    queryFn: () => layDSDanhMuc(soTrang, soLuong, trangThai, tuKhoa)
  })
  ...
  const [editingDanhMuc, setEditingDanhMuc] = useState<DanhMucFormData | undefined>(undefined)
  const [selectedRowId, setSelectedRowId] = useState<number | null>(null)
  const handleSetEditingDanhMuc = (data: DanhMuc) => {
    setEditingDanhMuc({
  ...
  const { mutate: deleteMutate, isPending: isDeleting } = useMutation({
    mutationFn: ({ id }: { id: number }) => xoaDanhMuc(id),
    onSuccess: (data) => {
    ...
    <div className='relative min-h-screen w-full bg-gray-50 p-4 flex flex-col'>
      ...
      <div className='flex-1 grid grid-cols-1 lg:grid-cols-3 gap-6 w-full'>
        <div className='lg:col-span-2 bg-white rounded-lg border border-gray-200 p-6 w-full shadow-sm flex flex-col'>
          ...
          <div
            className='flex-1 rounded-lg flex flex-col mb-3 mt-3 overflow-auto max-h-[550px] [&::-webkit-scrollbar]:w-2
  [&::-webkit-scrollbar-track]:rounded-full
  [&::-webkit-scrollbar-track]:bg-gray-100
  [&::-webkit-scrollbar-thumb]:rounded-full
  [&::-webkit-scrollbar-thumb]:bg-gray-300
  dark:[&::-webkit-scrollbar-track]:bg-red-700
                ...
                {ketQua.length === 0 ? (
                  ...
                  ketQua.map((s, i) => (
                    ...
                    >
                      ...
                      <TooltipTableCell text={s.tenDanhMuc} index={i} className='text-base py-3' />
                      <TableCell className='text-center border text-base py-0.5 border-gray-200 border-b '>
