...
import { Trash2, Search, ChevronsUpDownIcon, Check, Star } from 'lucide-react'
...
import { layDSBaoCaoDanhGia, xoaBaoCaoDanhGia } from '@/apis/baocaodanhgia'
...
import BaoCaoDanhGiaForm from '@/components/Form/BaoCaoDanhGiaForm'
export function QuanLyBaoCaoDanhGia() {
  ...
  const { data, refetch, isLoading } = useQuery({
    ...
    queryFn: () =>
      layDSBaoCaoDanhGia(soTrang, soLuong, tuKhoa, ngayBatDauQuery, ngayKetThucQuery, idLoaiBaoCao, trangThai)
  })
  ...
  const { mutate: deleteMutate, isPending: isDeleting } = useMutation({
    mutationFn: ({ id }: { id: number }) => xoaBaoCaoDanhGia(id),
    onSuccess: (data) => {
  ...
  const [selectedLoaiDanhGia, setSelectedLoaiDanhGia] = useState<LoaiDanhGia | null>(null)
  ...
  return (
    <div className='relative min-h-screen w-full bg-gray-50 p-4 flex flex-col'>
      ...
      <div className='flex-1 grid grid-cols-1 lg:grid-cols-3 gap-4 w-full'>
                ...
                {ketQua.length === 0 ? (
                  ...
                  ketQua.map((s, i) => (
                    ...
                    >
                      ...
                      <TooltipTableCell text={s.noiDungDanhGia} index={i} className='text-sm py-2' />
                      <TableCell className='text-center border text-sm py-0.5 border-gray-200 border-b'>
                        <div className='flex items-center justify-center gap-1 h-full text-sm'>
                          <span>{s.soDiemDanhGia}</span>
        ...
        <div className='bg-white rounded-lg border p-4 w-full shadow-sm'>
          <BaoCaoDanhGiaForm
