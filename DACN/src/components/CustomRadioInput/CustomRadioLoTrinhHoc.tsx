...
export function CustomRadioGroupLoTrinhHoc({ options, loTrinhFilter }: RadioGroupProps) {
  const { pathname } = useLocation()

  return (
    <div className='flex gap-1'>
      {options.map((option) => {
        const url = createURLLoTrinh(pathname, {
          ...loTrinhFilter,
          trangThai: Number(option.value)
        })

        const isActive = String(loTrinhFilter.trangThai) === option.value
